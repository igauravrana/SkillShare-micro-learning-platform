...
const TutorialDetails = ({ tutorial, onBack, user }) => {
  ...
  const handleLike = async () => {
    ...
    console.log("Sending learnerId as:", user?.uid || user?.email);

    try {
      const response = await fetch(`http://localhost:5000/api/tutorials/${tutorial._id}/like`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ learnerId: user?.uid || user?.email })
      });
  ...
  return (
    ...
    >
      ...
      <h1 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>{tutorialData.title}</h1>
      ...
      <div style={{ fontSize: '1.1rem', lineHeight: '1.6', color: '#333', marginBottom: '1.5rem' }}>
        <p>{tutorialData.description}</p>
      </div>

      <div style={{ marginBottom: '2rem' }}>
        <button
          onClick={handleLike} className="like-btn"
          style={{
